...
import { FiFileText, FiArrowLeft, FiUpload,FiClock,FiBarChart2,FiBox,FiRotateCw } from 'react-icons/fi'
...
function SecondProbleme({ setProblemDisplay }) {
    ...
    const download =()=>{
        if(result){
            ...
            for (let index = 0; index < result.sequence.length; index++) {
                data += "Task Number :" + result.sequence[index][0] + "\t , Processing Time : "+result.sequence[index][1]+ "\t , Due  date : "+result.sequence[index][2]+"\t , alpha   : "+result.sequence[index][3]+"\t , Beta : "+result.sequence[index][4]+"\n";
            }
    ...
    return (
        <div className="container  mx-auto h-48 p-10">
            ...
            <div>
                ...
                <div>
                <p className="font-jost my-2 mx-2 font-semibold text-gray-500">Probleme 2</p>
                </div>
            </div>
            {file === null  ?
            ...
            {/* {
                loading?(
                    <div className="w-10 h-10 bodrer-3 border-gray-500 rounded-3xl spin">

                    </div>
                ):(<></>)
            } */}

            {file !== null && result==null && !loading ?
        ...
        {result?
        (<Gantt problem={2} data={result.sequence}></Gantt>):(
            <></>
        )}
        </div>
    )
